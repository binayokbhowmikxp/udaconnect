...
4. `kubectl apply -f deployment/udaconnect-persons-api.yaml` - Set up the service and deployment for the persons API
5. `kubectl apply -f deployment/udaconnect-locations-api.yaml` - Set up the service and deployment for the locations API
6. `kubectl apply -f deployment/location_producer.yaml` - Set up the service and deployment for the locations producer gRPC to put message in Kafka
7. `kubectl apply -f deployment/location_consumer.yaml` - Set up  deployment for the locations consumer gRPC server to consume messages from Kafka
...
* `http://localhost:30001/api/persons` - Base path for persons API
* `http://localhost:30003/api/locations` - Base path for locations API
